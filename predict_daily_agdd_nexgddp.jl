...
function main()
    ...
    for year in start_year : end_year
        ...
        for rcp in rcps
            ...
            for cmip in cmips
                println(cmip)
                grow_count += process_model(cmip, rcp, year, ttop, tbase, agdd_threshold, data_dir)
            end
        end
...
@everywhere function calc_agdd_growth(gdd_ts::Array{Float64,1}, agdd_threshold::Float64)
    grow_range = gdd_ts.>0
    [sum(cumprod(grow_range[i:(i+364)]) .* gdd_ts[i:(i+364)]) > agdd_threshold for i in 1:365]
end


function process_model(cmip::String, rcp::String, year::Int64, ttop::Float64, tbase::Float64, agdd_threshold::Float64, data_dir::String)
    ...
    pmap(x -> calc_agdd_growth(x, agdd_threshold), [gdd[i,t,:] for i in 1:size(gdd)[1], t in 1:size(gdd)[2]])
end
